...
import { 
  Lock, Unlock, Package, Shield, ShieldCheck, User, Mail, AlertCircle, CheckCircle, ArrowRight, Smartphone, Monitor
} from 'lucide-react';
import { 
...
import { 
  doc, getDoc, setDoc, updateDoc, onSnapshot, collection, addDoc, query, where, getDocs 
} from 'firebase/firestore';
import { auth, db, googleProvider } from '../config/firebase.js';
import { appId, EMAILJS_CONFIG, ADMIN_EMAILS } from '../config/constants.js';
import { formatToPeruTime, formatToPeruDate, getFriendlyErrorMessage } from '../utils/helpers.js';
...
export const GatekeeperView = ({ onValidate, showToast }) => {
...
export const AuthView = ({ allowedType, onBack, setUser, gatekeeperEnabled, showToast }) => {
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      ...
      if (typeof showToast === 'function') {
        showToast('Código enviado. Expira en 10 min.');
      }
    } catch (err) {
  ...
  const handleVerifyCode = async () => {
    ...
    if (verificationCode && userEnteredCode === verificationCode) {
      ...
      setError('');
      if (typeof showToast === 'function') showToast('Correo verificado.');
      return;
    }
  ...
  const handleFinishGoogleRegistration = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      if (typeof showToast === 'function') {
        showToast('¡Registro exitoso! Ya puedes acceder a la tienda.');
      }
    } catch (err) {
      console.error(err);
