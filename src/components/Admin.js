...
import { 
  DollarSign, BarChart3, Users, Package, Settings, Plus, X, ImageIcon, Server, UploadCloud, Monitor, ShieldCheck, Lock, Shield, Clock, FileText, Star, Trash2, Globe, Smartphone, Landmark, AlertCircle 
} from 'lucide-react';
import { 
  doc, onSnapshot, getDoc, setDoc, updateDoc, collection, addDoc, deleteDoc, query, where, getDocs, writeBatch 
} from 'firebase/firestore';
import { ref, uploadBytesResumable, getDownloadURL, deleteObject } from 'firebase/storage';
import { db, storage } from '../config/firebase.js';
import { appId, EMAILJS_CONFIG } from '../config/constants.js';
import { formatToPeruDate, formatToPeruTime, secureAdminAction } from '../utils/helpers.js';

export const AdminPanel = ({ products, banners, showToast, editingProduct, setEditingProduct, setPreviewProduct, setActiveTab, user }) => {
    ...
    if (isFormOpen) {
      const timer = setTimeout(() => {
        ...
        if (infoEditorRef.current && !infoQuillInstance.current) {
          infoQuillInstance.current = new Quill(infoEditorRef.current, {
            theme: 'snow',
  ...
  const handleDeleteOrder = async (order) => {
    await secureAdminAction(user, async () => {
      if (!confirm(`¿Estás seguro de eliminar el pedido de "${order.productTitle}" para ${order.userEmail}? Esta acción lo borrará permanentemente.`)) return;
      try {
  ...
  const handleClearVisitorLogs = async () => {
    await secureAdminAction(user, async () => {
      if (!confirm("¿Estás seguro de que deseas limpiar todo el registro de visitas?")) return;
      try {
        const batchSize = 500;
  ...
  const handleSubmit = async (e) => {
    ...
    await secureAdminAction(user, async () => {
      ...
      try {
        ...
        setFormData({ title: '', description: '', price: '', imageUrl: '', installInstructions: '', hasOffer: false, offerPrice: '', isVisible: true });
  ...
  const handleToggleVisibility = async (product) => {
    await secureAdminAction(user, async () => {
      ...
      } catch (error) { showToast("Error al cambiar la visibilidad.", "error"); }
    });
  };

  const handleDeleteClient = async (client) => {
    await secureAdminAction(user, async () => {
      if (!confirm(`¿Estás seguro de eliminar al cliente "${client.fullName}"?`)) return;
      try {
      ...
      } catch (error) { showToast("Error al intentar eliminar al cliente por completo.", "error"); }
    });
  };
  ...
  const handleToggleClientStatus = async (client) => {
    await secureAdminAction(user, async () => {
      ...
      if (!confirm(`¿Estás seguro de ${newStatus === 'active' ? 'habilitar' : 'inhabilitar'} a "${client.fullName}"?`)) return;
      try {
  ...
  const handleDeleteReview = async (review) => {
    await secureAdminAction(user, async () => {
      ...
      } catch (error) { showToast("No se pudo eliminar la reseña.", "error"); }
    });
  };
  ...
  const handleBannerSubmit = async (e) => {
    ...
    await secureAdminAction(user, async () => {
      ...
      try {
        ...
        const bannerData = { ...bannerFormData, imageUrl: finalImageUrl, imageStoragePath: finalImageStoragePath || null, updatedAt: new Date().toISOString() };
        if (editingBanner) {
      ...
      } catch (error) { showToast("Error al guardar el banner.", "error"); } finally { setIsSubmittingBanner(false); }
    });
  };
  ...
  const handleDeleteBanner = async (banner) => {
    await secureAdminAction(user, async () => {
      ...
      } catch (error) { showToast("No se pudo eliminar el banner.", "error"); }
    });
  };

  useEffect(() => {
    if (editingBanner) {
      setBannerFormData({ title: editingBanner.title || '', subtitle: editingBanner.subtitle || '', imageUrl: editingBanner.imageUrl || '', link: editingBanner.link || '', active: editingBanner.active !== undefined ? editingBanner.active : true, bgColor: editingBanner.bgColor || '#3b82f6', position: editingBanner.position || 'top' });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex gap-2 sm:gap-4 p-1 bg-slate-200 dark:bg-slate-800 w-full sm:w-fit rounded-xl overflow-x-auto whitespace-nowrap scrollbar-hide">
        <button onClick={() => setAdminTab('products')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'products' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Productos</button>
        <button onClick={() => setAdminTab('banners')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'banners' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Banners</button>
        <button onClick={() => setAdminTab('orders')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all flex items-center gap-2 text-sm sm:text-base ${adminTab === 'orders' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
        ...
        <button onClick={() => setAdminTab('reviews')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'reviews' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Reseñas</button>
        <button onClick={() => setAdminTab('clients')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'clients' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Clientes</button>
        <button onClick={() => setAdminTab('visitors')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'visitors' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Visitas</button>
        <button onClick={() => setAdminTab('security')} className={`px-4 sm:px-6 py-2 rounded-lg font-bold transition-all text-sm sm:text-base ${adminTab === 'security' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Seguridad</button>
      </div>
      ...
      {adminTab === 'banners' && (
        <div className="space-y-8">
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 animate-in fade-in zoom-in duration-300">
              <div className="flex justify-between items-center mb-8 pb-4 border-b border-slate-100 dark:border-slate-700">
                <h2 className="text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                  <div className="bg-blue-600 text-white p-2 rounded-xl shadow-lg">{editingBanner ? <Settings size={24} /> : <Plus size={24} />}</div>
              ...
              <form onSubmit={handleBannerSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input value={bannerFormData.subtitle} onChange={e=>setBannerFormData({...bannerFormData, subtitle: e.target.value})} type="text" placeholder="Ej: Hasta 50% de descuento" className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-900 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none transition-all font-bold shadow-inner"/>
                  </div>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-end">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-black uppercase tracking-widest text-slate-400">Link de Destino</label>
                ...
                <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl">
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <span className="ml-3 text-sm font-black uppercase tracking-widest text-slate-500">Banner Activo</span>
                  </label>
                </div>
                    ...
                    <div key={banner.id} className="group bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-800 p-6 flex flex-col md:flex-row gap-6 items-center">
                      ...
                      <div className="flex-1 space-y-2 text-center md:text-left">
                        ...
                        <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                          <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md ${banner.active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{banner.active ? 'Activo' : 'Inactivo'}</span>
                          <span className="text-[10px] font-black uppercase px-2 py-1 rounded-md bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300">Posición: {banner.position === 'top' ? 'Superior' : banner.position === 'left' ? 'Izquierda' : 'Derecha'}</span>
                        </div>
                      </div>
            ...
            <div className="space-y-8">
              {isFormOpen ? (
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 animate-in fade-in zoom-in duration-300">
                  <div className="flex justify-between items-center mb-8 pb-4 border-b border-slate-100 dark:border-slate-700">
                    <h2 className="text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                      <div className="bg-blue-600 text-white p-2 rounded-xl shadow-lg">{editingProduct ? <Settings size={24} /> : <Plus size={24} />}</div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-3xl border border-blue-100 dark:border-blue-900/30">
                      <div className="space-y-4">
                        ...
                        <div className="relative">
                          ...
                          <input required value={formData.price} onChange={e=>setFormData({...formData, price: e.target.value})} type="number" step="0.01" className="w-full h-[76px] pl-12 pr-6 py-4 bg-white dark:bg-slate-900 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none transition-all text-xl font-black text-slate-600 dark:text-slate-300 shadow-sm"/>
                        </div>
                      </div>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <label className="text-sm font-black uppercase tracking-widest text-blue-600">Precio de Oferta (USD)</label>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={formData.hasOffer} onChange={e=>setFormData({...formData, hasOffer: e.target.checked})} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500"/>
                            <span className="text-xs font-black uppercase text-slate-500">Activar Oferta</span>
                          </label>
                        </div>
                        <div className={`relative transition-all duration-300 ${formData.hasOffer ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                          ...
                          <input required={formData.hasOffer} value={formData.offerPrice} onChange={e=>setFormData({...formData, offerPrice: e.target.value})} type="number" step="0.01" placeholder="0.00" className="w-full h-[76px] pl-12 pr-6 py-4 bg-white dark:bg-slate-900 border-2 border-blue-500 rounded-2xl outline-none transition-all text-xl font-black text-blue-600 shadow-lg shadow-blue-500/10"/>
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-end">
                      <div className="space-y-2">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900 h-[76px] px-6 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all cursor-pointer flex items-center">
                          <label className="flex items-center gap-4 cursor-pointer w-full">
                            <div className="bg-blue-100 dark:bg-blue-900/30 p-2 rounded-xl text-blue-600 shrink-0"><UploadCloud size={24} /></div>
                            <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-black uppercase tracking-widest text-slate-400">Información del Programa (Privado)</label>
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
                  <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <button onClick={() => setIsFormOpen(true)} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-black py-4 px-8 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-xl shadow-blue-500/20">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {products.map(p => (
                      <div key={p.id} className="group bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border border-slate-100 dark:border-slate-800 p-6 transition-all hover:shadow-2xl">
                        <div onClick={() => { setPreviewProduct(p); setActiveTab('store'); }} className="relative aspect-square bg-white dark:bg-slate-800 rounded-2xl overflow-hidden mb-6 cursor-pointer">
                        ...
                        <div className="flex gap-2 mt-4">
                          <button onClick={() => handleToggleVisibility(p)} className={`p-3 rounded-xl transition-all ${p.isVisible === false ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{p.isVisible === false ? <Unlock size={20} /> : <Lock size={20} />}</button>
                          <button onClick={() => setEditingProduct(p)} className="flex-1 bg-white dark:bg-slate-800 text-blue-600 border-2 border-blue-50 font-black py-3 rounded-xl text-xs uppercase tracking-widest">Editar</button>
                          <button onClick={() => handleDeleteProduct(p)} className="bg-red-50 dark:bg-red-900/10 text-red-500 p-3 rounded-xl"><X size={20} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {adminTab === 'orders' && (
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
              <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-8 flex items-center gap-2"><FileText size={24} className="text-slate-400" /> Pedidos Recibidos</h2>
                    ...
                    {allOrders.map(order => (
                      <tr key={order.id}>
                        ...
                        <td className="py-4 pr-4">
                          <p className="font-bold text-slate-800 dark:text-white text-sm">{allClients.find(c => c.uid === order.userId)?.fullName || order.userEmail}</p>
                          <p className="text-xs text-slate-400">{order.productTitle}</p>
                        </td>
                        ...
                        <td className="py-4">
                          <div className="flex items-center gap-2">
                            {order.status === 'pending' && <button onClick={() => handleApproveOrder(order)} className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white text-[10px] font-black uppercase px-4 py-2 rounded-xl shadow-lg">Aprobar</button>}
                            <button onClick={() => handleDeleteOrder(order)} className="p-2 bg-red-50 text-red-500 rounded-lg"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {adminTab === 'reviews' && (
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
              <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-8 flex items-center gap-2"><Star size={24} className="text-amber-400" /> Moderación de Reseñas</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800">
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Fecha</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Usuario</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Comentario</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Estado</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400 text-right">Acciones</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {allReviews.map(review => (
                      <tr key={review.id}>
                        ...
                        <td className="py-4 pr-4"><p className="font-bold text-slate-800 dark:text-white text-sm">{review.userName}</p></td>
                        <td className="py-4 pr-4"><p className="text-xs text-slate-600 dark:text-slate-400 italic">"{review.comment}"</p></td>
                        <td className="py-4 pr-4"><span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md ${review.isVisible === false ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{review.isVisible === false ? 'Oculto' : 'Visible'}</span></td>
                        <td className="py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => handleToggleReviewVisibility(review)} className={`p-2 rounded-lg ${review.isVisible === false ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{review.isVisible === false ? <Unlock size={16} /> : <Lock size={16} />}</button>
                            <button onClick={() => handleDeleteReview(review)} className="p-2 bg-red-50 text-red-500 rounded-lg"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {adminTab === 'clients' && (
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
              <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-8 flex items-center gap-2"><User size={24} className="text-slate-400" /> Clientes Registrados</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800">
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Nombres</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Email</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Estado</th>
                      <th className="pb-4 text-[10px] font-black uppercase tracking-wider text-slate-400 text-right">Acciones</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {allClients.map(client => (
                      <tr key={client.id}>
                        <td className="py-4 pr-4"><p className="font-bold text-slate-800 dark:text-white text-sm">{client.fullName}</p></td>
                        <td className="py-4 pr-4"><p className="text-sm text-slate-600 dark:text-slate-400">{client.email}</p></td>
                        <td className="py-4 pr-4"><span className={`text-[10px] font-black uppercase px-2 py-1 rounded-md ${client.status === 'disabled' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{client.status === 'disabled' ? 'Inhabilitado' : 'Activo'}</span></td>
                        <td className="py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => handleToggleClientStatus(client)} className={`p-2 rounded-lg ${client.status === 'disabled' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{client.status === 'disabled' ? <Unlock size={16} /> : <Lock size={16} />}</button>
                            <button onClick={() => handleDeleteClient(client)} className="p-2 bg-red-50 text-red-500 rounded-lg"><X size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {adminTab === 'visitors' && (
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-2"><Globe size={24} className="text-slate-400" /> Visitas</h2>
                <button onClick={handleClearVisitorLogs} className="bg-red-50 text-red-600 p-4 rounded-2xl"><Trash2 size={20} /></button>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="text-left border-b border-slate-100 dark:border-slate-700">
                      <th className="pb-4 text-[10px] font-black uppercase text-slate-400 px-4">Fecha</th>
                      <th className="pb-4 text-[10px] font-black uppercase text-slate-400 px-4">Usuario</th>
                      <th className="pb-4 text-[10px] font-black uppercase text-slate-400 px-4">IP</th>
                      <th className="pb-4 text-[10px] font-black uppercase text-slate-400 px-4 text-right">Acciones</th>
                    </tr>
                  </thead>
                  <tbody>
                    {visitorLogs.map(log => (
                      <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30">
                        <td className="py-4 px-4 text-sm font-bold">{formatToPeruDate(log.timestamp)}</td>
                        <td className="py-4 px-4 text-sm">{log.userEmail}</td>
                        <td className="py-4 px-4 font-mono text-blue-600">{log.ip}</td>
                        <td className="py-4 px-4 text-right">
                          <button onClick={() => handleBlockIP(log)} className={`p-2 rounded-xl ${blockedIPs.some(b => b.deviceId === log.deviceId) ? 'bg-red-500 text-white' : 'bg-slate-100 text-slate-400'}`}><Lock size={16} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {adminTab === 'security' && (
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 max-w-2xl mx-auto">
              <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-8 flex items-center gap-2"><Lock size={24} className="text-amber-400" /> Seguridad</h2>
              <div className="space-y-8">
                <div className={`p-6 rounded-2xl border flex items-center justify-between ${gatekeeperConfig.enabled ? 'bg-emerald-50' : 'bg-slate-50'}`}>
                  <div>
                    <p className="text-sm font-black uppercase tracking-tight">Acceso Restringido</p>
                    <p className="text-xs font-bold">{gatekeeperConfig.enabled ? 'ACTIVO' : 'DESACTIVADO'}</p>
                  </div>
                  <button onClick={handleToggleGatekeeper} className={`px-6 py-2.5 rounded-xl font-black text-xs uppercase ${gatekeeperConfig.enabled ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'}`}>{gatekeeperConfig.enabled ? 'Desactivar' : 'Activar'}</button>
                </div>
                <div className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border text-center">
                  ...
                  <button onClick={handleForceRegenerateCode} className="text-xs font-black text-blue-600 uppercase">Regenerar ahora</button>
                </div>
              </div>
            </div>
          )}
